...
This macro works similar to the `@Pyodide.eval`, but the code is only passed as a parameter.
...
manually in your `onload` macro.

To load additional packages in pyodide, use the `pyodide.loadPackage` function. This function returns a promise that resolves when the package is loaded. You can use the `await` keyword to wait for the package to load before using it. For example: `await pyodide.loadPackage('numpy')` will load the numpy package and make it available for use. You can also load multiple packages at once by passing a list of package names to `pyodide.loadPackage`. For example: `await pyodide.loadPackage(['numpy', 'pandas'])` will load both the numpy and pandas packages.
